...
const Avatar = ({ person, size = 'md' }) => {
  const sizeClasses = { sm: 'w-10 h-10 text-[9px]', md: 'w-14 h-14 text-[10px]', lg: 'w-16 h-16 text-xs', xl: 'w-24 h-24 text-sm' };
  return (
    <div className={`${sizeClasses[size]} rounded-full bg-gradient-to-br ${bgColors[person.name?.charCodeAt(0) % bgColors.length || 0]} flex items-center justify-center shadow-lg flex-shrink-0`}>
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-gray-900 dark:text-white text-sm leading-tight line-clamp-2 break-words">{salutation} {displayName}</h4>
      ...
      >
        <div className="p-6">
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <h3 className="mt-4 font-bold text-gray-900 dark:text-white text-lg text-center">{salutation} {displayName}</h3>
...
const ExecutiveMembersScreen = () => {
  ...
  return (
    <motion.div className="w-full pb-12" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      <motion.div
      ...
      >
        <div className="relative py-6 px-4">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white text-center">Executive Committee</h1>
      ...
      <AnimatePresence>
        {selectedPerson && <ExecutiveModal person={selectedPerson} onClose={() => setSelectedPerson(null)} />}
      </AnimatePresence>
    </motion.div>
  );
};
