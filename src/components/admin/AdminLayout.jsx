...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <aside className={`fixed top-0 left-0 z-40 h-full ${collapsed ? 'w-16' : 'w-64'} bg-white/95 dark:bg-slate-900/95 border-r border-gray-200 dark:border-white/10 transform transition-all duration-200 ease-in-out flex flex-col ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
          ...
          {!collapsed && (
            <Link to="/dashboard" className="flex items-center gap-3" onClick={() => setMobileOpen(false)}>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">SDSG Campus</p>
              </div>
            </Link>
          )}
          <button
        ...
        <ScrollableArea className={`flex-1 ${collapsed ? 'px-2' : 'px-3'} py-4`} hideTrack>
