...
export function AuthProvider({ children }) {
  ...
  const fetchUserProfile = useCallback(async (firebaseUser) => {
    if (!firebaseUser) { setUserProfile(null); setPermissions([]); profileLoadAttempted.current = false; return; }
    try {
      ...
      if (profile) {
        if (profile.status === USER_STATUS.INACTIVE || profile.status === USER_STATUS.SUSPENDED) {
          setUserProfile(null); setPermissions([]); await authService.signOut();
          setError('Your account has been deactivated. Contact your administrator.'); return;
        }
  ...
  const createUser = useCallback(async (email, password, userData) => {
    ...
    try {
      ...
      if (user) { await auditService.log(AUDIT_ACTIONS.USER_CREATED, user.uid, { targetUser: email, targetRole: userData.role, userEmail: user.email }); }
      return newUser;
    } catch (err) { const message = getAuthErrorMessage(err.code); setError(message); throw err; }
  }, [user]);
  ...
  const resetPassword = useCallback(async (email) => {
    ...
    try {
      ...
      if (user) { await auditService.log(AUDIT_ACTIONS.PASSWORD_RESET, user.uid, { targetEmail: email, userEmail: user.email }); }
    } catch (err) { const message = getAuthErrorMessage(err.code); setError(message); throw err; }
  }, [user]);
  ...
  const value = {
    user, userProfile, permissions, loading, error, isAuthenticated: !!user,
    isSuperAdmin, isAdmin, isStaff, isStudent,
    loginWithEmail, loginWithGoogle, logout, createUser, resetPassword,
    sendSignInLink, loginWithEmailLink, refreshProfile, clearError,
