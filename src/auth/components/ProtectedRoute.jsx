...
export function ProtectedRoute({ children, requiredPermissions, fallbackPath = '/login' }) {
  ...
  if (requiredPermissions && requiredPermissions.length > 0) {
    const hasAccess = requiredPermissions.mode === 'any' ? hasAnyPermission(requiredPermissions.permissions) : hasAllPermissions(requiredPermissions.permissions);
    if (!hasAccess) return <Navigate to="/unauthorized" replace />;
  }

  if (!canAccessRoute(location.pathname, userProfile?.role)) return <Navigate to="/unauthorized" replace />;
  return children;
}
...
export function RoleRoute({ children, roles, fallbackPath = '/login' }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>;
  if (!isAuthenticated) return <Navigate to={fallbackPath} state={{ from: location }} replace />;
  if (roles && !roles.includes(userProfile?.role)) return <Navigate to="/unauthorized" replace />;
  return children;
}
...
export function PublicRoute({ children }) { return children; }
